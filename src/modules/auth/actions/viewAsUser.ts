...
export async function viewAsUser(userId: string, companySlug: string, redirectUrl?: string) {
  ...
  const finalUrl = redirectUrl || `/${companySlug}/dashboard`
  console.log('[viewAsUser] Cookies set with 7-day expiry, redirecting to', finalUrl)
  redirect(finalUrl)
}
