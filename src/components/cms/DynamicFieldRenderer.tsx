...
function inferType(key: string, value: unknown): string {
  ...
  if (Array.isArray(value)) {
    return lk.includes('paragraph') || lk.includes('paragrafo') ? 'paragraphs' : 'array'
  }

  if (typeof value === 'object' && value !== null) {
    return lk.includes('cta') || lk.includes('button') || lk.includes('botao') ? 'cta' : 'object'
  }

  if (typeof value === 'string') {
    const v = value.toLowerCase()
    if (lk.includes('link') || lk.includes('url') || lk.includes('href') || lk.includes('destino')) return 'url'
    if (lk.includes('color') || v.startsWith('#')) return 'color'
    if (lk.includes('video') || v.endsWith('.mp4') || v.endsWith('.webm')) return 'video'
    if (
      lk.includes('image') || lk.includes('logo') || lk.includes('src') ||
      lk.includes('img') || lk.includes('bg') ||
      v.endsWith('.png') || v.endsWith('.jpg') || v.endsWith('.jpeg') ||
      v.endsWith('.avif') || v.endsWith('.webp')
    ) return 'image'
    if (lk.includes('icon')) return 'icon'
    if (
      value.length > 50 ||
      lk.includes('desc') || lk.includes('text') ||
      lk.includes('content') || lk.includes('body')
    ) return 'textarea'
  }
  ...
  if (type === 'boolean') {
    return (
      <div className="flex items-center gap-2">
        <Switch checked={Boolean(value)} onCheckedChange={(v) => onChange(path, v)} />
        <span className="text-xs text-brand-muted">{Boolean(value) ? 'Ativado' : 'Desativado'}</span>
      </div>
    )
  }
  ...
  if (type === 'number') {
    return (
      <input
        ...
        className="w-full bg-brand-bg border border-border rounded-lg px-3 py-2 text-sm text-brand-text focus:outline-none focus:ring-2 focus:ring-brand-primary"
      />
    )
  }
  ...
  if (type === 'url') {
    return (
      <input
        ...
        className="w-full bg-brand-bg border border-border rounded-lg px-3 py-2 text-sm text-cyan-400 font-mono placeholder:text-brand-muted focus:outline-none focus:ring-2 focus:ring-brand-primary"
        placeholder="https://..."
      />
    )
  }
      ...
      <div className="space-y-2">
        ...
        >
          ...
          <span className="text-sm text-brand-muted">
            {isUploading ? 'Enviando...' : str ? 'Trocar imagem' : 'Adicionar imagem'}
          </span>
        </button>
        {str && (
          <input
      ...
      <div className="space-y-2">
        ...
        >
          ...
          <span className="text-sm text-brand-muted truncate">
            {isUploading ? 'Enviando...' : str ? 'Trocar vídeo' : 'Adicionar vídeo'}
          </span>
        </button>
        {str && (
      ...
      const clear = (v: unknown): unknown => {
        ...
        if (typeof v === 'object' && v !== null)
          return Object.fromEntries(
            Object.entries(v as Record<string, unknown>).map(([k, w]) => [k, clear(w)]),
          )
        return ''
      }
