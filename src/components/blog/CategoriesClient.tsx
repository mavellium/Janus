...
function DeleteCategoryButton({ id }: { id: string }) {
  ...
  function handleDelete() {
    ...
    startTransition(async () => { await deleteBlogCategory(id) })
  }
...
export function CategoriesClient({ categories, projectId }: CategoriesClientProps) {
  ...
  return (
    <div className="p-8 w-full">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-brand-text">Categorias</h1>
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        {categories.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 gap-3">
            <FolderOpen size={40} className="text-brand-muted opacity-40" />
            <p className="text-sm text-brand-muted">Nenhuma categoria criada</p>
          </div>
        ) : (
          ...
          <table className="w-full min-w-[600px]">
            <thead>
              <tr className="border-b border-border">
                <th className="text-left px-5 py-3 text-xs font-semibold text-brand-muted uppercase tracking-wide">Categoria</th>
            ...
            <tbody className="divide-y divide-border">
              {categories.map((cat) => (
                <tr key={cat.id} className="hover:bg-brand-btn-light/20 transition">
                  ...
                  <td className="px-5 py-3">
                    <code className="text-xs text-brand-primary bg-brand-primary/10 px-2 py-0.5 rounded">{cat.slug}</code>
                  </td>
                  <td className="px-5 py-3 text-sm text-brand-muted">{cat.description ?? '—'}</td>
                  <td className="px-5 py-3">
      ...
      {modal === 'create' && (
        <CategoryModal projectId={projectId} open onClose={() => setModal(null)} />
      )}
      {modal && modal !== 'create' && (
        <CategoryModal projectId={projectId} category={modal} open onClose={() => setModal(null)} />
      )}
    </div>
  )
}
