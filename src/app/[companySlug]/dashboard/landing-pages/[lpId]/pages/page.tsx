...
export default async function LandingPagePagesPage({
  ...
  const canCreate = await checkPermission(session, 'PAGE_CREATE', 'landingPages', 'page')
  const canBuild = await checkPermission(session, 'PAGE_BUILD', 'landingPages', 'page')
  const canDelete = await checkPermission(session, 'PAGE_DELETE', 'landingPages', 'page')
