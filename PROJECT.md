...
- **Actions:** `registerUser.ts` — cria usuário com bcrypt hash e `companyId` = default company | `signInAction.ts` — roteia por empresas **vinculadas** (`UserCompany`): 0 → `/no-company`, 1 → `/{slug}/dashboard`, 2+ → `/select-company`; `companyId` primário só como fallback sem vínculo | `updatePreferences.ts` — persiste preferências de UI no banco (grava no usuário efetivo/impersonado) | `restartOnboarding.ts` — reseta `preferences.onboarding` p/ pending e redirect server-side ao dashboard (grava no usuário efetivo/impersonado) | `updateAvatar.ts` — atualiza avatar com URL da BunnyCDN
- **Queries:** `getUserByEmail.ts` — busca usuário ativo por email (sem deletedAt) | `getUserPreferences.ts` — busca preferências do usuário logado | `getUserCompanies.ts` — lista empresas **vinculadas** via `UserCompany` (primário só como fallback sem vínculo)
...
### notifications
- **Entidade:** `src/modules/notifications/domain/release.ts` — `SystemRelease` (tagName, name, bodyHtml sanitizado, publishedAt, htmlUrl, prerelease)
- **Actions:** `refreshReleases.ts` — expira o cache das releases via `updateTag('github-releases')` (Next 16; `revalidateTag` agora exige profile) após auth | `markNotificationsSeen.ts` — grava `preferences.notifications_last_seen_at` (read-merge-write no JSON)
- **Queries:** `getReleases.ts` — `getSystemReleases(page=1)` busca releases do GitHub paginado (`RELEASES_PER_PAGE=20`, media type `html+json`, cache `revalidate: 3600`, tag `github-releases`, `GITHUB_TOKEN` opcional) | `getCurrentVersion()` — tag da última release estável | `countUnreadReleases(userId)` — nº de releases mais novas que `notifications_last_seen_at`
...
### seo
- **Entidade:** `src/modules/seo/domain/seoCheck.ts` — `SeoCheckResult`/`PageSignals`/`SeoAnalysisResult`/`GeoFoundationSignals`/`GeoFoundationResult` | `seoScoring.ts` — `scoreSeo()` puro, rubrica de 14 checks somando 100 (testado em `seoScoring.spec.ts`) | `geoFoundationScoring.ts` — `scoreGeoFoundation()` puro, 3 checks automatizados (acesso de robôs de IA no robots.txt, Organization/LocalBusiness, FAQPage) somando 100 + 1 item editorial manual sem pontuação (testado em `geoFoundationScoring.spec.ts`, task 15 fase 3)
- **Infra:** `fetchTargetPage.ts` — página + robots.txt (corpo exposto em `robotsTxtBody`) + sitemap via safe-fetch | `parseHtml.ts` — extração cheerio (title, meta, headings, OG, JSON-LD incl. `jsonLdTypes` via @type/@graph, alt, palavras)
- **Scan de site (todas as páginas):** `domain/siteScan.ts` — `mergeSiteUrls()` (une Pages publicadas do DB + URLs do sitemap, mesma origem, dedup, teto 20), `aggregateSiteScan()` (média SEO/GEO das páginas ok), `computeCommonIssues()` (checks reprovados agregados por chave/severidade) | `infra/fetchSitemapUrls.ts` — sitemap.xml + sitemapindex (cheerio xmlMode) | `actions/analyzeSite.ts` — Zod → auth → guard empresa → resolve `project.previewUrl` (fallback p/ previewUrl de página) → rate limit 10/dia por site → coleta URLs → robots.txt 1× → fetch concorrente (pool 5) + score por página → persiste 1 `SeoAnalysis` com `projectId` setado e `checks.site = SiteScanData`
- **Actions:** `analyzeSeoUrl.ts` — Zod → auth → guard empresa → rate limit 20/dia → fetch SSRF-safe (via `infra/fetchTargetPage.ts`: em 403/429 retenta com UA de navegador e, se ainda bloqueado, degrada com `contentAccessible:false`/html vazio em vez de erro) → parse → score SEO + score GEO foundation → persiste `SeoAnalysis.checks` como `{ seo, geoFoundation, contentAccessible }` (análise avulsa: `projectId` fica null)
- **Queries:** `getSeoAnalysis.ts` — por id escopado a companyId, `parseStoredChecks()` lê tanto o formato novo `{seo,geoFoundation,contentAccessible}` quanto o array legado pré-GEO (`contentAccessible` ausente → `true`) | `getRecentSeoAnalyses.ts` — histórico avulso (`projectId: null`, não vaza scans de site) | `getRecentSiteScans.ts`/`getSiteScan.ts` — scans por `projectId`, lêem `checks.site`
- **UI:** `CombinedScoreHeader.tsx`/`ScoreRing.tsx`/`useCountUp.ts` — os 2 scores (SEO/GEO) animados e clicáveis (scroll direto pro checklist); `SeoScoreCard.tsx` só checklist (sem anel próprio); `SiteScanButton.tsx` (client, useActionState → `analyzeSite` → push p/ relatório), `SiteScanReport.tsx` (client — header agregado + problemas recorrentes + lista de páginas com drill-down por página), `SiteSeoLanding.tsx`/`SiteScanReportView.tsx` (server async compartilhados por sites e landing-pages)
- **Docs:** `.claude/context/seo/`

### geo
- **Entidade:** `src/modules/geo/domain/geoProbe.ts` — `GeoTargetProfileContext` (name/aliases/description/industry/location/website/targetAudience/differentiators), `GeoProbeAdapter`/`GeoProbeResult`/`IagScoreInput`/`IagScoreResult`, `LAYER_WEIGHTS` (DECISAO 3, AVALIACAO 2, PROBLEMA 1) e labels PT-BR
- **Domain:** `detectMention.ts` — `detectMention()`/`normalizeForMatching()` puros, correspondência de nome+apelidos sem acento com limite de palavra (heurística, não NLP; testado em `detectMention.spec.ts`) | `calculateIagScore.ts` — `calculateIagScore()` puro, IAG Score 0-100 por média ponderada da taxa de menção por pergunta, exclui execuções com erro, calcula share of voice (testado em `calculateIagScore.spec.ts`) | `buildProbeInstructions.ts` — `buildProbeContextNote()` puro, monta o prompt de sistema a partir do contexto do perfil (setor/local/público), **nunca inclui o nome da empresa** (evita viés; testado em `buildProbeInstructions.spec.ts`)
- **Infra:** `infra/providers/shared.ts` — `buildGeoSystemInstructions` (re-export de `buildProbeContextNote`), timeout e `estimateCostUsdCents()` compartilhados | `openaiProbe.ts` — Responses API (`responses.create`, tool `web_search` no LIVE_SEARCH, extrai `url_citation`) | `geminiProbe.ts` — `@google/genai` `models.generateContent` (tool `googleSearch` no LIVE_SEARCH, citações via `groundingMetadata.groundingChunks[].web.uri`) | `perplexityProbe.ts` — REST `POST /chat/completions` com fetch+AbortController (citações via `citations`/`search_results`, **custo real** em `usage.cost.total_cost`) | `infra/providers/index.ts` — registry `getProbeAdapter()`/`isProviderConfigured()`/`getEnabledProviders()`/`getConfiguredProviders()`; **CLAUDE no enum sem adapter**. Nenhum adapter lança: erro vira `errorMessage` e sai do cálculo. Todo `probeQuestion()` recebe `profile: GeoTargetProfileContext`
- **Actions:** `guard.ts` — `requireAdmin()` (role ADMIN + usuário efetivo via impersonação) | `manageGeoProfiles.ts` — `createGeoTargetProfile`/`updateGeoTargetProfile`/`archiveGeoTargetProfile` (soft delete `archivedAt`), **sem vínculo com Company** — qualquer negócio pode ser cadastrado, estilo perfil de conta do HubSpot | `runRaioX.ts` — orquestra `perguntas × provedores × modos`, cooldown 1h/perfil, teto 25 perguntas, monta `GeoTargetProfileContext` e passa a cada `probeQuestion()`, `$transaction` (snapshot + probe runs) + `logAudit` | `manageGeoQuestions.ts` — cria/edita/arquiva (soft delete `ARCHIVED`) | `manageGeoCompetitors.ts` — cria/remove concorrente. Todas as 4 usam `profileId` (não `companyId`)
- **Queries:** `getGeoTargetProfiles.ts` — `getGeoTargetProfiles()`/`getGeoTargetProfileById()` | `getGeoTargetQuestions.ts` — perguntas ACTIVE por prioridade | `getGeoCompetitors.ts` — concorrentes + apelidos | `getGeoSnapshot.ts` — `getLatestGeoSnapshot()`/`getGeoSnapshotHistory()`/`getGeoProbeRunsBySnapshot()`. Todas recebem `profileId`
- **Escopo:** só painel admin (`/dashboard-admin/geo`); **custo variável real** = `perguntas × provedores marcados × 2 modos` (até 150 chamadas pagas/execução com 3 provedores e o teto de 25 perguntas)
- **Docs:** `.claude/context/geo/`

### billing
- **Entidade:** `Subscription` (Prisma) — 1:1 com `Company`; tier (TRIAL/INICIAL/MEDIO/PRO/ENTERPRISE), status, trialEndsAt, discountPercent/discountEndsAt, `limitOverrides` (Json = `Partial<PlanLimits>`), notes
- **Domain:** `plans.ts` — **fonte única** do catálogo (`PLAN_CATALOG`, `PUBLIC_PLANS`, `PlanLimits`, `LIMIT_LABELS`, `TRIAL_DAYS=7`, `POST_TRIAL_DISCOUNT_PERCENT=20`/`_MONTHS=3`); `null` = ilimitado | `limits.ts` — `resolveSubscription()` (plano + override, trava cotas de criação quando EXPIRED/CANCELED), `resolveEffectiveStatus()` (trial vencido é **computado no relógio**, sem job), `parseLimitOverrides()`, `initialSubscriptionData()` (testado em `limits.spec.ts`, 11 casos)
- **Guards:** `guards/enforcePlan.ts` — `enforceQuota(companyId, key)` (só consulta uso se houver teto) e `enforceFeature(companyId, key)`; 402 = plano não inclui/travado, 429 = teto estourado
- **Queries:** `getCompanySubscription.ts` — assinatura resolvida (fallback INICIAL + warn se faltar linha) | `getCompanyUsage.ts` — uso atual (projects, users, SEO 24h, scans 24h, Raio-X no mês). A listagem admin vem de `modules/admin/queries/getAdminCompanies.ts`, que já devolve `subscription` + `usage` por empresa
- **Actions:** `adminUpdateSubscription.ts` — `adminUpdateSubscription` (plano/status/datas/desconto), `adminUpdateLimitOverrides` (aumentar ou reduzir limite por empresa), `adminConvertTrial` (encerra teste → INICIAL com 20% por 3 meses), `adminExtendTrial`; todas ADMIN-only e auditadas
- **Enforcement:** createProject (`projects`), analyzeSeoUrl (`seoAnalysesPerDay`), analyzeSite (`siteScansPerDay`), runRaioX (`geoRunsPerMonth`, só se o perfil tem companyId), dev/createUser (`users`), create/updateBlogPost (`blogScheduling`, só com data futura), createScript (`scripts`). **ADMIN não é barrado** — é o override manual
- **Docs:** `.claude/context/billing/`

### companies
- **Queries:** `getRecentCompanyActivity.ts` — últimos AuditLogs da empresa (filtra `companyId`, `impersonatedId: null`, entidades Project/Page/Blog*) p/ feed da home; param opcional `userId` escopa à atividade de um usuário (dashboard passa o usuário efetivo/impersonado)
...
- **Actions (login/senha):** `signInWithGoogle.ts` — grava preferência de sessão e chama `signIn('google', { redirectTo: '/' })` | `requestPasswordReset.ts` — rate limit (5/15min por IP, 3/15min por e-mail), cria token de uso único (sha256, TTL 30min) e envia e-mail; resposta genérica anti-enumeração | `resetPassword.ts` — valida token + `passwordSchema`, grava bcrypt(12), zera `requiresPasswordReset` e invalida tokens | `setGoogleAuthEnabled.ts` — liga/desliga login Google da própria conta (desligar apaga o vínculo) | `unlinkGoogleAccount.ts` — remove `UserOAuthAccount` do Google
- **Domain:** `passwordReset.ts` — token/hash/TTL | `passwordResetEmail.ts` — template HTML+texto | `loginErrors.ts` — mapa `?error=<code>` → mensagem pt-BR
- **Queries:** `getCompanyUsers.ts` — usuários ativos de uma empresa (id, name, email, role), ordenados por name | `getUserAuthMethods.ts` — status do Google (disponível/ativo/vínculo) p/ o painel | `getPasswordResetTokenStatus.ts` — valida token da URL e devolve o e-mail
- **Docs:** `.claude/context/auth/`

---

## Componentes

- `src/components/auth/LoginForm.tsx` — Client — formulário de login com useActionState + checkIpStatus, countdown regressivo (MM:SS), overlay bloqueio com cor #514030; checkbox "Lembrar minha conta", link "Esqueceu a senha?" e botão Google (props `googleEnabled`/`defaultEmail`/`defaultRemember`/`providerError`/`passwordResetDone`)
- `src/components/auth/GoogleSignInButton.tsx` — Client — submit secundário do form de login (`formAction={signInWithGoogle}` + `formNoValidate`), pending via `useFormStatus`
- `src/components/auth/ForgotPasswordForm.tsx` — Client — pede e-mail e exibe a confirmação genérica de envio
- `src/components/auth/ResetPasswordForm.tsx` — Client — nova senha + confirmação; ao concluir redireciona p/ `/login?reset=1`
- `src/components/auth/ConnectedAccountsCard.tsx` — Client — card "Contas conectadas": switch de login com Google, e-mail vinculado, datas e botão Desvincular
- `src/components/dev/DevSidebar.tsx` — Client — sidebar colapsável exclusiva do Dev Dashboard; links para dashboard, empresas, usuários, notificações e configurações do dev; rodapé com `Versão: {currentVersion}`
- `src/components/admin/AdminSidebar.tsx` — Client — sidebar colapsável exclusiva do Admin Panel; links para `/dashboard-admin/*` (dashboard, empresas, desenvolvedores, usuários, Raio-X IA, logs, notificações, configurações); rodapé com `Versão: {currentVersion}`
- `src/components/notifications/NotificationsFeed.tsx` — Client — feed de releases em timeline (linha vertical + dots, agrupado por mês/ano), busca por texto, filtros Todas/Estáveis/Pré-releases, "Carregar mais" paginado via `/api/notifications/releases`; cards com tag, badge Atual/Pré-release, data pt-BR, notas sanitizadas
- `src/components/notifications/NotificationsContent.tsx` — Server async — busca releases + contagem não lidas em paralelo; renderiza MarkNotificationsSeen + NotificationsFeed (usado dentro de Suspense)
- `src/components/notifications/NotificationsSkeleton.tsx` — Server — skeleton animate-pulse (toolbar + 3 cards) como fallback do Suspense
...
- `src/components/seo/SeoScoreCard.tsx` — Server-compatible — checklist (checks reprovados por severidade + aprovados; prop `expanded` mostra pontos); sem anel próprio, score fica só no `CombinedScoreHeader`
- `src/components/seo/ScoreRing.tsx` — Client — anel SVG único (cor por faixa 50/80) com número animado via `useCountUp`
- `src/components/seo/useCountUp.ts` — Client hook — contagem 0→valor com easing (~900ms), usado pelo ScoreRing
- `src/components/seo/CombinedScoreHeader.tsx` — Client — 2 `ScoreRing` lado a lado (SEO/GEO); cada um é um botão que faz `scrollIntoView` até a seção de checklist correspondente (`geoScore: null` → placeholder "Indisponível" p/ análises pré-GEO)
- `src/components/seo/SeoUrlInputForm.tsx` — Client — input de URL (normaliza sem protocolo p/ https), useActionState → `analyzeSeoUrl`, resultado inline via `CombinedScoreHeader` + 2 `SeoScoreCard` (SEO/GEO) + `scrollIntoView` automático ao concluir + link p/ relatório
...
- `src/components/onboarding/OnboardingTour.tsx` — Client — tour de boas-vindas fullscreen (10 passos): welcome → spotlight nos itens da Sidebar (`data-tour`, box-shadow 200vmax + pulso, tooltip ancorado; fallback centralizado em mobile) → análise SEO/GEO interativa → conclusão com confete; teclado (Esc pula, ←/→/Enter navegam), barra de progresso, persiste `preferences.onboarding` (`pending`/`skipped`/`completed` + step p/ retomar) via `updatePreferences`; skip mostra toast "refazer em Configurações" e só persiste ao fechar
- `src/components/onboarding/onboardingSteps.ts` — dados dos 10 passos do tour (kind, target `data-tour`, ícone, título, descrição)
- `src/components/onboarding/SeoGeoPreview.tsx` — Client — passo interativo do tour: input de URL → `analyzeSeoUrl` real → skeleton com varredura `.onb-scan` + mensagens cíclicas → 2 `ScoreRing` (SEO/GEO) + top 3 recomendações em cascata
- `src/components/onboarding/Confetti.tsx` — Client — 28 peças CSS determinísticas (keyframe `onbConfettiFall`), sem lib externa
- `src/components/billing/PlanUsageCard.tsx` — Server — card do dashboard da empresa: barras de uso vs. limite (sites, usuários, Raio-X), aviso de fim de teste e de assinatura inativa
- `src/components/billing/CompanyPlanModal.tsx` — Client — modal ADMIN de plano/situação/datas/desconto/notas (`adminUpdateSubscription`)
- `src/components/billing/CompanyLimitsModal.tsx` — Client — modal ADMIN de override manual por limite (número, `ilimitado` ou "seguir o plano")
- `src/components/billing/planDisplay.ts` — labels/estilos de status, listas de chaves de limite e formatação de data compartilhadas pelos dois modais
- `src/components/ui/form-select.tsx` — Client — `Select` do Radix para formulário (h-9, largura total, submete por `name`); substitui `<select>` nativo, cujo dropdown o SO desenha ignorando o tema
- `src/components/ui/date-field.tsx` — Client — `input[type=date]` com ícone de calendário via `showPicker()`, botão de limpar e indicador nativo do WebKit escondido
...
- `src/components/dashboard/Sidebar.tsx` — Client — sidebar colapsável com useState(initialCollapsed) + startTransition; logo 48px→28px; toggle PanelLeftClose/PanelLeftOpen; avatar next/image + fallback UserCircle; estado persistido via updatePreferences em background; item Notificações → `/[companySlug]/dashboard/notifications`; MAIN_ITEMS inclui "Análise de SEO" → `/[companySlug]/dashboard/seo`; rodapé exibe `Versão: {currentVersion}` (prop vinda do layout via `getCurrentVersion()`)
...
- `src/components/landing/useInView.ts` — Client hook — `useInView()` (IntersectionObserver, dispara uma vez e desconecta), `usePrefersReducedMotion()` (via `useSyncExternalStore`, SSR-safe) e `useIsomorphicLayoutEffect`
- `src/components/landing/Reveal.tsx` — Client — wrapper de entrada por scroll; alterna `data-lp-reveal` e aceita `delay` para cascata
- `src/components/landing/LandingNav.tsx` — Client — nav fixa da home pública; fundo **sólido** (`lp-nav-blur`) após 12px de scroll, âncoras de seção (inclui `#precos`), drawer mobile (Esc fecha), logo light/dark por classe
- `src/components/landing/HeroPipeline.tsx` — Client — demo animada em 2 colunas: à esquerda os campos do Janus sendo preenchidos por um cursor de mouse (título digitado em ritmo humano, cor escolhida entre 4 amostras da paleta, foto com barra de upload, texto do botão, Publicar); à direita o preview do site montando ao vivo **ou** o JSON da API, alternados pelo botão "Ver na API"; arte da foto é SVG inline sobre `var(--brand-*)`; pausa fora da viewport e mostra estado final com `prefers-reduced-motion`
- `src/components/landing/ScorePanel.tsx` — Client — painel de SEO/GEO da home pública; reusa `ScoreRing` (monta só ao entrar na viewport) + checklist com os labels reais do scoring
- `src/components/landing/IagPanel.tsx` — Client — painel do Raio-X: IAG Score com `useCountUp` + barras de share of voice animadas por largura
- `src/components/landing/ApiTabs.tsx` — Client — 3 abas com os endpoints reais de `/api/v1/content` e a resposta JSON; botão de copiar endpoint
- `src/components/landing/SectionHeading.tsx` — Server — cabeçalho editorial de seção (índice mono + filete + eyebrow + título + descrição)
- `src/components/landing/FaqList.tsx` — Server — FAQ em `<details>` nativo com animação CSS
- `src/components/landing/LandingFooter.tsx` — Server — rodapé da home pública com logo, colunas de âncoras (inclui `#precos`) e link de acesso

---

## Páginas

- `src/app/page.tsx` — Server — **landing pública do produto** (hero animado, faixa de clientes `CLIENTS`, CMS, SEO/GEO, Raio-X de IA, operação, fluxo, API, Planos `PLANS`/`#precos`, FAQ, CTA); com sessão ativa redireciona para `/dev/[id]/dashboard` (DEVELOPER), `/dashboard-admin` (ADMIN), `/no-company` (sem empresa) ou `/{companySlug}/dashboard`. O middleware já redireciona logado antes de renderizar. Planos (Inicial/Médio/Pro/Enterprise) e feature-list por card são placeholder de negócio — validar preço e limites reais antes de divulgar; CTA do Enterprise ainda aponta para `/login` por falta de canal de vendas dedicado. **Docs:** `.claude/context/landing/`
...
- `src/app/[companySlug]/dashboard/seo/[analysisId]/page.tsx` — Server — relatório completo de SEO (score expandido com pontos, metadados, ReanalyzeButton, histórico da empresa) + seção "Fundação para GEO" (quando `geoFoundation` presente); `notFound()` se análise não pertence à empresa
- `src/app/[companySlug]/dashboard/seo/page.tsx` — Server — índice de Análise de SEO (SeoUrlInputForm + histórico de até 20 análises da empresa); acessível via item "Análise de SEO" na Sidebar
...
- `src/app/dashboard-admin/geo/page.tsx` + `AdminGeoClient.tsx` — Raio-X de Visibilidade em IA: seletor de empresa, IAG Score (`ScoreRing`), perguntas sem menção, comparação com concorrentes, CRUD de perguntas-alvo/concorrentes, histórico; botão desabilitado sem `OPENAI_API_KEY`
...
- `src/app/(auth)/login/page.tsx` — tela de login (Server Component) — lê `?error`/`?reset` e os cookies de preferência para alimentar o `LoginForm`
- `src/app/(auth)/forgot-password/page.tsx` — Server — solicitação do link de redefinição (e-mail pré-preenchido pelo cookie)
- `src/app/(auth)/reset-password/[token]/page.tsx` — Server — valida o token e exibe o formulário de nova senha ou a tela "Link inválido ou expirado"
...
- **User** (`users`) — id (UUID), email (unique), password (text), role (**DEFAULT/ADMIN/DEVELOPER**), image (String?), preferences (Json? default {}), **companyId (UUID?, nullable fk→companies SET NULL)**, **createdById (UUID?, id do criador)**, requiresPasswordReset (bool), **googleAuthEnabled (bool, default true — autoriza o provider Google para a conta)**, permissions (String[]), createdAt, updatedAt, deletedAt
...
- **Subscription** (`subscriptions`) — id (UUID), **companyId (UUID, unique, fk→companies CASCADE — 1:1)**, tier (enum `PlanTier`: TRIAL|INICIAL|MEDIO|PRO|ENTERPRISE, default TRIAL), status (enum `SubscriptionStatus`: TRIALING|ACTIVE|PAST_DUE|CANCELED|EXPIRED, default TRIALING), trialEndsAt?, currentPeriodEnd?, discountPercent?, discountEndsAt?, **limitOverrides (Json default {} — `Partial<PlanLimits>`, override manual do admin)**, notes?, createdAt, updatedAt | índices [tier], [status], [trialEndsAt]. Empresas anteriores à migração entraram como ENTERPRISE/ACTIVE (grandfathered)
...
- **UserOAuthAccount** (`user_oauth_accounts`) — id (UUID), userId (fk→users CASCADE), provider (`'google'`), providerAccountId (sub), providerEmail?, linkedAt, lastLoginAt? | @@unique([provider, providerAccountId]) e @@unique([userId, provider]) — vínculo escrito pelo callback `signIn`; **não** é o model `Account` do Auth.js (sem tokens)
- **PasswordResetToken** (`password_reset_tokens`) — id (UUID), userId (fk→users CASCADE), tokenHash (unique, sha256 — token em claro só no e-mail), expiresAt (indexed, TTL 30min), usedAt? (uso único), ip?, userAgent?, createdAt
...
- **GeoTargetProfile** (`geo_target_profiles`) — id (UUID), **companyId? (UUID?, fk→companies CASCADE)**, name, description?, industry?, location?, website?, aliases (String[]), targetAudience?, differentiators?, createdById? (fk→users **SET NULL**), archivedAt? (soft delete), createdAt, updatedAt | índices [archivedAt], [companyId]. `companyId` é **opcional**: preenchido = cliente do Janus e cada execução desconta da cota `geoRunsPerMonth` do plano; nulo = análise interna de qualquer negócio (estilo conta HubSpot), sem cota
- **GeoTargetQuestion** (`geo_target_questions`) — id (UUID), profileId (fk→geo_target_profiles CASCADE), text, layer (enum `GeoQuestionLayer`: DECISAO|AVALIACAO|PROBLEMA), priority (Int), status (enum `GeoQuestionStatus`: ACTIVE|ARCHIVED), createdAt, updatedAt | índice [profileId+status]
- **GeoCompetitor** (`geo_competitors`) — id (UUID), profileId (fk→geo_target_profiles CASCADE), name, aliases (String[]), createdAt, updatedAt
- **GeoProbeRun** (`geo_probe_runs`) — id (UUID), profileId (CASCADE), snapshotId? (fk→geo_score_snapshots **SET NULL**), targetQuestionId (CASCADE), provider (enum `GeoProvider`: OPENAI|GEMINI|PERPLEXITY|CLAUDE), mode (enum `GeoProbeMode`: MODEL_MEMORY|LIVE_SEARCH), rawResponse (Text), companyMentioned (bool), mentionedCompetitorId? (**SET NULL**), citedUrls (String[]), costUsdCents?, errorMessage?, createdAt
- **GeoScoreSnapshot** (`geo_score_snapshots`) — id (UUID), profileId (CASCADE), userId?, score (Int 0-100, IAG Score), breakdown (Json), competitorComparison (Json?), totalCostUsdCents (Int), createdAt | índice [profileId+createdAt]
...
- `src/lib/security/safe-fetch.ts` — `safeFetch()` SSRF-safe p/ URLs de usuário: só http/https, DNS resolvido com bloqueio de IP privado/loopback/link-local/CGNAT (v4+v6), redirects manuais validados por hop (máx 3), timeout 10s, corpo máx 3MB, 1 retry em falha de rede, `dns.setDefaultResultOrder('ipv4first')`; `SafeFetchError` tipado com `.cause` preservada (INVALID_URL/PRIVATE_ADDRESS_BLOCKED/TIMEOUT/TOO_LARGE/TOO_MANY_REDIRECTS/BLOCKED_BY_TARGET/UNREACHABLE)
...
- `src/lib/auth.ts` — NextAuth v5 com **lazy init** (`NextAuth(async () => config)`): Credentials + Google (registrado só se houver `AUTH_GOOGLE_ID/SECRET`), JWT strategy, `session.maxAge` vindo do cookie de "lembrar minha conta", callback `signIn` que só aceita Google para `User` ativo com `googleAuthEnabled` (e grava `UserOAuthAccount`) e callback `jwt` que recarrega o usuário real do banco. **Sem `PrismaAdapter`** — os models `Account`/`Session`/`VerificationToken` nunca existiram no schema
- `src/lib/auth/session.ts` — cookies `janus.remember_session` (30d × 8h) e `janus.remembered_email`; `resolveSessionMaxAge`, `saveSessionPreference`, `readSessionPreference`
- `src/lib/auth/oauth.ts` — `GOOGLE_PROVIDER_ID`, `isGoogleAuthConfigured()`
- `src/lib/mail.ts` — `sendMail()` via API HTTP do Resend (sem SDK); sem `RESEND_API_KEY`/`MAIL_FROM` imprime no console em dev e falha explicitamente em produção
- `src/lib/app-url.ts` — `appBaseUrl()` (`AUTH_URL`/`NEXTAUTH_URL` ou headers `x-forwarded-*`) p/ montar links de e-mail
...
- `src/app/globals.css` — bloco `lp-*` da landing: keyframes (`lpFadeUp`, `lpCaret`, `lpDrift`, `lpPing`, `lpSweep`), tipografia (`.lp-display`, `.lp-mono` sobre as vars Geist), translucidez via `color-mix` (`.lp-tint-*`, `.lp-border-*`, `.lp-glow-*`, `.lp-nav-blur`) — **os modificadores de opacidade do Tailwind (`bg-x/10`) não funcionam neste projeto**, pois as cores são `var(--…)` cruas e o Tailwind 3 não aplica alpha nelas; `scroll-behavior: smooth` escopado por `:root:has(.lp-landing)`; tudo neutralizado em `prefers-reduced-motion`
...
| :--------- | :-------------------------------------------- | :--------------------------------------------------------- |
| 2026-08-22 | `.github/workflows/deploy.yml`, `docker-compose.yml` | **INFRA(deploy):** build migrado do VPS para as runners da GitHub com push no GHCR (`ghcr.io/mavellium/janus:{prod,test}`, uma imagem por `NEXT_PUBLIC_URL_API`), cache de layers via `type=gha` e matrix paralela. Servidor agora só faz `git pull` + `docker compose pull` + `up -d` (sem `down` — zero downtime, DB não reinicia); novo secret obrigatório: `GHCR_TOKEN` (PAT `read:packages`) |
| 2026-08-22 | `.claude/context/billing/**` (novo), `.claude/README.md`, `PROJECT.md` | **DOCS(billing):** módulo documentado — `_index.md`, `domain.md` (catálogo, 6 regras absolutas, estados, migração), `enforcement.md` (mapa de onde cada limite é aplicado + o que ficou sem guard de propósito), `patterns.md`, `changelog.md` |
| 2026-08-22 | `src/components/ui/form-select.tsx` (novo), `src/components/ui/date-field.tsx` (novo), `src/components/billing/*Modal.tsx`, `src/app/dashboard-admin/geo/AdminGeoClient.tsx`, `src/modules/geo/actions/manageGeoProfiles.ts` | **UI:** `<select>` nativo (dropdown desenhado pelo SO, ilegível no dark) trocado pelo `Select` do Radix via `FormSelect` — altura `h-9` igual aos `Input`, submete por `name` (Radix renderiza `<select>` oculto dentro de `<form>`). `DateField` novo: ícone de calendário próprio via `showPicker()`, botão de limpar e indicador nativo do WebKit escondido. **Radix proíbe `value=""`** em `SelectItem`, então o "sem empresa" do Raio-X virou o sentinela `'none'`, traduzido para `null` no `companyIdSchema` |
| 2026-08-22 | `src/app/dashboard-admin/companies/AdminCompaniesClient.tsx`, `src/modules/admin/queries/getAdminCompanies.ts`, `src/components/billing/{planDisplay.ts,CompanyPlanModal.tsx,CompanyLimitsModal.tsx}` (novos) | **REFACTOR(planos — tela única):** a tela separada `/dashboard-admin/plans` foi **removida** (e o item da sidebar) — plano é atributo de empresa, então tudo passou a viver em Empresas. `getAdminCompanies` devolve `subscription` + `usage` (tipo `AdminCompanyRow`); a tabela ganhou coluna Plano fixa, colunas opcionais Raio-X/Teste/Desconto/Ajustes, filtros de plano/situação/ajustes e 4 ações por linha (plano, limites, estender teste, converter). Colunas Usuários/Projetos passaram a mostrar uso/limite em vez de contagem crua. Correções de padrão: wrapper `p-8`, tokens `brand-*` (não shadcn), botões de ação crus `w-3.5`, filtros com opção "Todos", e `QuotaValue` mostrando só o número quando não há teto (o "4/Ilimitado" era confuso) |
| 2026-08-22 | `src/components/billing/PlanUsageCard.tsx` (novo), `src/app/[companySlug]/dashboard/page.tsx` | **FEAT(planos — visão do cliente):** `PlanUsageCard` no dashboard da empresa com barras de uso vs. limite, aviso de fim de teste e de assinatura inativa |
| 2026-08-22 | `src/modules/projects/actions/createProject.ts`, `src/modules/seo/actions/{analyzeSeoUrl,analyzeSite}.ts`, `src/modules/blog/actions/{create,update}BlogPost.ts`, `src/modules/scripts/actions/createScript.ts`, `src/modules/dev/actions/createUser.ts`, `src/modules/geo/actions/runRaioX.ts` | **FEAT(planos — enforcement):** 8 actions passaram a consultar o plano antes de escrever. Os limites hardcoded de SEO (20/dia) e site scan (10/dia por projeto) saíram do código e vieram do catálogo — **site scan virou por empresa**, não por projeto, porque cota de plano é comercial. Agendamento de artigo só é cobrado quando `publishedAt` é futuro (publicar agora não consome feature paga) |
| 2026-08-22 | `prisma/schema.prisma`, `prisma/migrations/20260821120000_billing_plans_and_geo_company`, `src/modules/billing/**` (novo), `src/modules/admin/actions/revertAuditAction.ts` | **FEAT/SCHEMA(planos):** enums `PlanTier`/`SubscriptionStatus` + model `Subscription` (1:1 Company) + `GeoTargetProfile.companyId` (liga o Raio-X à cota do cliente). Migração faz backfill das empresas existentes como **ENTERPRISE/ACTIVE** (grandfathered — rebaixar quebraria cliente em produção). Empresa nova nasce em TRIAL de 7 dias via `initialSubscriptionData()` nas 3 actions de criação. `Subscription` registrada em `ENTITY_DELEGATES` para reversão de auditoria |
| 2026-08-22 | `src/modules/geo/actions/manageGeoProfiles.ts`, `src/app/dashboard-admin/geo/**`, `src/modules/geo/queries/getGeoTargetProfiles.ts` | **FEAT+FIX(Raio-X):** perfil do Raio-X pode ser vinculado a uma empresa cliente (select no modal; sem vínculo = análise interna sem cota). **FIX:** o wizard salva contexto via `updateGeoTargetProfile` sem enviar `companyId` e o schema transforma `undefined → null` — isso desvincularia a empresa silenciosamente a cada execução; a escrita agora fica atrás de `formData.has('companyId')` |
| 2026-08-21 | `src/app/page.tsx` | **UI/FEAT(landing):** logo real da Tegbe (`CLIENTS` agora `{name, logo}`, `next/image unoptimized`, chip `bg-neutral-900` pq o SVG é wordmark branco); cards de planos com slot de altura fixa pro selo, `min-h-12` na descrição e divisor antes do preço (alinha as 4 colunas); "Tudo do X" movido pra primeiro item da feature-list; limites de site passaram a citar landing pages também |
| 2026-08-21 | `src/app/page.tsx` | **FIX(landing/planos):** teste grátis de 7 dias restrito ao plano Inicial — Médio/Pro passam a ter CTA "Assinar agora" em vez de "Começar teste grátis"; badge "7 dias grátis" só no card Inicial; pill do cabeçalho corrigida ("no plano Inicial" em vez de "em qualquer plano") |
| 2026-08-21 | `src/app/page.tsx`, `src/components/landing/LandingNav.tsx`, `src/components/landing/LandingFooter.tsx` | **FEAT(landing):** duas seções novas na home pública — faixa de clientes logo após o hero (`CLIENTS`, hoje só "Tegbe") e seção de Planos (`#precos`, index 08) com 4 cards: Inicial R$97, Médio R$197, Pro R$397 (destacado "Mais popular"), Enterprise "Consulta"; badge "7 dias grátis — sem cartão de crédito". FAQ recuou de index 08 para 09; link "Planos" adicionado à nav e ao rodapé. Preços e feature-list por plano são placeholder de negócio (o usuário pediu ajuda para definir considerando custo variável de IA do Raio-X (`src/modules/geo/`) + hospedagem + margem) — validar antes de divulgar publicamente. CTA "Falar com vendas" do Enterprise aponta para `/login` por não existir canal de contato dedicado ainda |
| 2026-08-16 | `src/components/auth/ForgotPasswordForm.tsx`, `src/modules/auth/actions/requestPasswordReset.ts`, `src/app/(auth)/forgot-password/page.tsx`, `src/app/(auth)/reset-password/[token]/page.tsx` | **UX(recuperar acesso):** confirmação deixou de ser só a mensagem genérica — mostra o e-mail de destino (pega erro de digitação, a falha mais comum do fluxo), validade do link, dica de spam, "Enviar novamente" (mesma action com `email` em hidden) e "Digitei o e-mail errado" (volta ao form sem recarregar). A action passou a devolver `email` em **todas** as saídas, inclusive quando a conta não existe, senão a tela de confirmação viraria oráculo de existência de usuário; erro de rate limit agora informa os minutos reais do `resetAt`. TTL do link vem de `PASSWORD_RESET_TTL_MINUTES` via prop (não pode ser importado no cliente: o módulo usa `node:crypto`) |
| 2026-08-16 | `src/components/auth/AuthLogo.tsx` (novo), `LoginForm.tsx`, `ForgotPasswordForm.tsx`, `ResetPasswordForm.tsx` | **UI(auth):** placeholder "J" das três telas de autenticação trocado pela logo real (`logo-min.svg` / `janus-logo-min-white.svg`, as mesmas do sidebar colapsado) em um componente compartilhado que alterna claro/escuro por `dark:` |
| 2026-08-16 | `.claude/context/auth/**` (novo), `.claude/README.md`, `PROJECT.md` | **DOCS(auth):** módulo documentado via skill `module-docs` — `_index.md`, `domain.md`, `actions.md`, `queries.md`, `patterns.md`, `changelog.md`. Registra as duas decisões que não se leem no código: por que o `PrismaAdapter` saiu e por que `NextAuth()` precisa ser função |
| 2026-08-16 | `src/app/[companySlug]/dashboard/settings/*`, `src/app/dev/[devId]/dashboard/settings/*`, `src/app/dashboard-admin/settings/page.tsx`, `src/components/auth/ConnectedAccountsCard.tsx` | **FEAT(painel — contas conectadas):** card em Ajustes › Segurança com switch "Entrar com Google" (`User.googleAuthEnabled`), e-mail vinculado, data do vínculo/último acesso e botão Desvincular; sai do ar quando o provider não está configurado no ambiente. `DevSettingsClient` (usado por dev e admin) e `SettingsClient` passaram a receber `authMethods` + `settingsPath`. Toda mudança gera audit log |
| 2026-08-16 | `src/modules/auth/{actions,queries,domain}/**`, `src/lib/mail.ts`, `src/lib/app-url.ts`, `src/app/(auth)/forgot-password/page.tsx`, `src/app/(auth)/reset-password/[token]/page.tsx`, `src/components/auth/{ForgotPasswordForm,ResetPasswordForm}.tsx` | **FEAT(esqueci minha senha):** fluxo self-service completo — token de 32 bytes guardado só como sha256, TTL 30min, uso único, rate limit por IP e por e-mail, resposta genérica anti-enumeração e audit log no pedido e na troca. E-mail pela API HTTP do Resend (`RESEND_API_KEY`/`MAIL_FROM`), sem dependência nova; sem as chaves, em dev o link cai no console do servidor. Não revoga sessões JWT já emitidas |
| 2026-08-16 | `src/lib/auth.ts`, `src/lib/auth/session.ts`, `src/modules/users/actions/signInAction.ts`, `src/components/auth/LoginForm.tsx`, `src/app/(auth)/login/page.tsx` | **FEAT(login — Google + lembrar minha conta):** `NextAuth()` virou lazy (`async () => config`) porque `session.maxAge` agora depende do cookie `janus.remember_session` (30d marcado × 8h desmarcado — o mesmo valor governa `exp` do JWT e o `expires` do cookie); cookie `janus.remembered_email` pré-preenche o e-mail. Provider Google só é registrado se houver `AUTH_GOOGLE_ID/SECRET`, exige `profile.email_verified`, **nunca cria usuário** (rejeita com `/login?error=<code>` antes de qualquer escrita) e grava o vínculo em `UserOAuthAccount`. **`PrismaAdapter` removido:** os models `Account`/`Session`/`VerificationToken` nunca existiram no schema e o `linkAccount` do adapter recebe o `expires_in` do Google, que o Prisma rejeita |
| 2026-08-16 | `prisma/schema.prisma`, `prisma/migrations/20260816120000_auth_google_and_password_reset` | **SCHEMA:** `User.googleAuthEnabled` (bool default true) + models `UserOAuthAccount` (vínculo OAuth por usuário/provider) e `PasswordResetToken` (hash, expiração, uso único). Migração aplicada no banco local |
| 2026-08-16 | `.claude/context/landing/**` (novo), `.claude/README.md`, `PROJECT.md` | **DOCS(landing):** módulo documentado via skill `module-docs` — `_index.md`, `domain.md` (conteúdo, máquina de estados da animação, tempos, tokens `lp-*`), `components.md` (10 componentes + seções da página), `patterns.md` (snippets + as 3 armadilhas: opacidade Tailwind sobre `var()`, `overflow-x-hidden` × header fixo, `setState` em effect) e `changelog.md`. Sem `actions.md`/`queries.md`: a landing não tem mutação nem leitura de banco (só `auth()` para o redirect). Índice adicionado em `.claude/README.md` |
| 2026-08-16 | `src/components/landing/HeroPipeline.tsx`, `src/app/page.tsx` | **UX(landing — hero com alternador API + ritmo humano):** painel do hero em 2 colunas (`sm:grid-cols-2`) — **1 · Você monta no Janus** e **2 · O site, ao vivo** — com botão **Ver na API** no cabeçalho da direita que troca o preview do site pelo JSON (`GET /api/v1/content/acme/home` + `200 · 9–14 ms` pulsando a cada commit), alimentado pelo mesmo estado da animação; volta com **Ver no site**. Tentativa anterior de 3 colunas fixas foi revertida: não cabia sem espremer a coluna de texto. Digitação reescrita com ritmo humano — 95 ms base + jitter determinístico (`(i*37)%5*18`) + pausa extra de 90 ms após espaço (~55 WPM, antes eram 38 ms fixos ≈ 300 WPM); demais tempos dobrados (swatch 420 ms, upload 130 ms, holds 550–700 ms), ciclo total ~14 s. Título encurtado p/ "Obra entregue no prazo" para o passo não arrastar |
| 2026-08-16 | `src/app/page.tsx`, `src/components/landing/HeroPipeline.tsx`, `src/components/landing/useInView.ts` | **FIX(landing — nav sem fundo ao rolar):** o `overflow-x-hidden` que eu tinha posto na raiz fazia o `overflow-y` computar como `auto` (regra do CSS Overflow: qualquer valor ≠ `visible` num eixo força o outro a `auto`), transformando a div raiz no contêiner de rolagem; com isso o documento nunca rolava, `window.scrollY` ficava em 0 e o `scrolled` da `LandingNav` jamais virava `true` — o fundo sólido estava correto no CSS mas a classe nunca era aplicada. Trocado por `overflow-x-clip`, que recorta sem criar contexto de rolagem. **Cuidado ao usar `overflow-x-hidden` em wrappers de página com header fixo dependente de scroll.** Hero: cursor do mouse agora percorre os 4 passos e o botão Publicar (posicionado por `getBoundingClientRect` num layout effect com refs por alvo, sem state), com ripple de clique a cada campo concluído; novo `useIsomorphicLayoutEffect` em `useInView.ts` para não avisar no SSR |
| 2026-08-16 | `src/app/globals.css`, `src/components/landing/LandingNav.tsx`, `src/components/landing/HeroPipeline.tsx` | **FIX+UX(landing — cabeçalho e hero):** `.lp-nav-blur` era `color-mix` a 82% + `backdrop-blur`, e o conteúdo transparecia sob a nav fixa ao rolar; agora é `var(--brand-bg)` sólido com sombra baixa (blur removido, virou inútil). Hero ganhou 3 elementos: cursor do mouse (`MousePointer2`) que percorre as amostras de cor e dispara um ripple no clique, moldura de navegador (3 pontos + domínio) sobre o preview para diferenciar "painel" de "site", e desfecho de publicação — botão Publicar acende quando os 4 campos estão preenchidos e a barra inferior vira `no ar · publicado agora` antes do ciclo reiniciar |
| 2026-08-16 | `src/components/landing/HeroPipeline.tsx`, `src/app/page.tsx`, `src/components/landing/*` (todos), `src/app/globals.css` | **UX(landing — animação + responsividade):** o painel do hero tinha scroll horizontal (o JSON não cabia) e mostrava JSON, que não diz nada para o público final. Refeito como uma montagem ao vivo: à esquerda os campos do Janus sendo preenchidos (título digitado, cor escolhida entre 4 amostras da paleta, foto com barra de upload, texto do botão), à direita a página do site montando em tempo real — a foto entra, o título aparece letra a letra e o botão assume a cor escolhida. Zero `overflow-x` no hero. Arte da foto = SVG inline (interior arquitetônico com janela, luz e mobiliário) desenhado só com `var(--brand-*)` e gradientes, sem asset externo e adaptando ao dark mode. Ritmo médio/lento (~10 s por ciclo: digitação 38 ms/char, upload 80 ms/tick, pausas de 480–700 ms). Responsividade completa em toda a landing: `overflow-x-hidden` na raiz, tipografia e paddings escalonados em 3 faixas (360/768/1280), alvos de toque ≥ 44 px no mobile, colunas do rodapé em grid 2× no celular, painéis do hero empilhando abaixo de `sm` |
| 2026-08-16 | `src/app/page.tsx`, `src/components/landing/HeroPipeline.tsx`+`FaqList.tsx`+`ApiTabs.tsx`+`IagPanel.tsx`+`LandingNav.tsx`+`LandingFooter.tsx` | **UX(landing — público final):** copy reescrita para quem usa o painel, não para quem o constrói. Removidos multi-tenant/`/slug`, CORS, req/min, SDK, AVIF, schema/UI Schema, front-end e demais detalhes de arquitetura sem valor para o leitor; seção da API mantida mas rotulada "Para quem desenvolve". H1 passou de "Você entrega o site" (agência) para "Seu site muda quando você decide" (cliente), com todas as seções, FAQ e nav alinhadas ao mesmo interlocutor. Animação do hero refeita mais limpa e rápida: formulário sendo preenchido de um lado, JSON acompanhando em tempo real do outro, barra do endpoint respondendo em ~20 ms (fase de "salvando" removida, digitação de 24→16 ms) |
| 2026-08-16 | `src/app/page.tsx`, `src/components/landing/**` (novo), `src/app/globals.css` | **FEAT(landing):** raiz `/` deixou de ser só um redirecionador e virou a página de venda pública do Janus (pt-BR), mantendo o redirecionamento de sessão ativa. Identidade existente preservada (paleta brand quente, logos `janus-logo*.svg`, tokens semânticos) com tipografia Geist Sans/Mono já carregada no layout. Conteúdo 100% ancorado no produto real: endpoints de `/api/v1/content` com a resposta verdadeira, labels reais do `seoScoring`/`geoFoundationScoring`, pesos do IAG Score e limites reais (14 checks, 20 páginas/varredura, 60 req/min, 60 dias de auditoria) — sem prova social inventada. Animações sem dependência nova: entrada por `IntersectionObserver`, demo do fluxo CMS→API digitando no hero, anéis de score reusando `ScoreRing`/`useCountUp`, barras de share of voice; tudo neutralizado em `prefers-reduced-motion`. Translucidez feita com `color-mix` em utilitários `lp-*` porque `bg-x/10` não funciona sobre as vars brand no Tailwind 3 |
| 2026-08-08 | `schema.prisma` (model `GeoTargetProfile` novo, `companyId`→`profileId` em 4 models), `geo/domain/geoProbe.ts`+`buildProbeInstructions.ts` (novo), `geo/infra/providers/shared.ts`+`openaiProbe.ts`+`geminiProbe.ts`+`perplexityProbe.ts`, `geo/actions/manageGeoProfiles.ts` (novo)+`manageGeoQuestions.ts`+`manageGeoCompetitors.ts`+`runRaioX.ts`, `geo/queries/getGeoTargetProfiles.ts` (novo)+demais queries, `revertAuditAction.ts`, `dashboard-admin/geo/page.tsx`+`AdminGeoClient.tsx`, `components/ui/textarea.tsx` (novo), `prisma/migrations/20260808214051_geo_target_profile` | **FEAT(geo — perfil de empresa desacoplado de Company):** a empresa analisada no Raio-X deixou de exigir um `Company` cadastrado no Janus. Novo model `GeoTargetProfile` (name, description, industry, location, website, aliases, targetAudience, differentiators) — perfil "estilo conta do HubSpot", preenchido manualmente pelo admin para qualquer negócio. `GeoTargetQuestion`/`GeoCompetitor`/`GeoProbeRun`/`GeoScoreSnapshot` migraram de `companyId` para `profileId` (CASCADE). Novo `buildProbeContextNote()` monta o prompt de sistema a partir do contexto do perfil (setor/local/público) e é usado pelos 3 adapters via `buildGeoSystemInstructions()` — **nunca inclui o nome da empresa no prompt**, para não enviesar a resposta simulada de compra. UI reescrita: seletor de perfil substitui o de empresa, modal de criação/edição com formulário completo de contexto, empty state para cadastrar a primeira análise. Migration `20260808214051` trocou as colunas sem risco — tabelas GEO estavam vazias (feature nunca usada em produção), confirmado por contagem antes de aplicar |
| 2026-08-06 | `geo/infra/providers/shared.ts`+`geminiProbe.ts`+`perplexityProbe.ts` (novos), `providers/index.ts`, `openaiProbe.ts`, `runRaioX.ts`, `dashboard-admin/geo/page.tsx`+`AdminGeoClient.tsx`, `.env.example` | **FEAT(geo — Gemini e Perplexity):** Raio-X passou de 1 para 3 provedores. Gemini via SDK oficial `@google/genai` (`models.generateContent`, LIVE_SEARCH com `tools:[{googleSearch:{}}]`, citações de `groundingMetadata.groundingChunks[].web.uri`); Perplexity via REST `POST /chat/completions` com fetch+AbortController (sem SDK — API compatível com formato OpenAI), citações de `citations`/`search_results` e **custo real** de `usage.cost.total_cost` em vez de estimativa. Instruções de sistema/timeout/estimador extraídos p/ `shared.ts`; `runRaioX` agora usa `getConfiguredProviders()` como default (422 se nenhuma chave); UI ganhou checkbox por provedor (desabilitado sem chave) + contagem estimada de consultas. **Custo por execução subiu p/ até `perguntas × 3 × 2` = 150 chamadas pagas** com o teto de 25 perguntas. Ressalva documentada: `sonar` sempre busca na web, então MODEL_MEMORY na Perplexity é aproximação, não equivalente aos outros. CLAUDE segue sem adapter |
| 2026-08-06 | `prisma/migrations/20260515024917_add_guest_tables`, `20260519999999_add_blog_and_script_tables`, `20260523999998_add_missing_company_project_page_columns`, `20260523999999_add_missing_user_columns`, `20260716000000_fix_developer_role_and_cascade_fks` (novas) | **FIX(prisma — histórico de migrações quebrado, pré-existente):** `migrate dev` falhava com P3006/P3018 ("relation guest_posts does not exist") porque várias tabelas/colunas foram criadas via `prisma db push` sem migração correspondente — banco real íntegro, histórico irreproduzível no shadow database. 5 migrations de reparo idempotentes (`IF NOT EXISTS` / `DO $$ EXCEPTION WHEN duplicate_object $$`) recriam `guest_entries`/`guest_posts`, as 5 tabelas de blog + `site_scripts` + enums, 13 colunas de companies/projects/pages, 5 colunas de users (incl. `permissions`) e realinham enum `DEVELOPER` + FKs RESTRICT→CASCADE. Aplicadas com `migrate resolve --applied` — **nenhum dado perdido** (o `migrate reset` sugerido pelo Prisma foi evitado). Validado por replay completo em banco descartável: histórico e banco real batem em 195 colunas, zero divergência |
| 2026-08-06 | `schema.prisma`, `modules/geo/**` (novo módulo), `dashboard-admin/geo/page.tsx`+`AdminGeoClient.tsx` (novos), `AdminSidebar.tsx`, `revertAuditAction.ts`, `.env.example`, `.claude/context/geo/**` | **FEAT(geo — Raio-X de Visibilidade em IA, só painel admin):** implementação da Fase 1 da task 15 a partir do `guia_implementacao_visibilidade_ia.md` — 4 models novos (`GeoTargetQuestion`/`GeoCompetitor`/`GeoProbeRun`/`GeoScoreSnapshot`) + 4 enums; domain puro `detectMention()` (match de nome/apelidos sem acento com limite de palavra — heurística explícita, não NLP) e `calculateIagScore()` (0-100 ponderado por camada de intenção, DECISAO 3 / AVALIACAO 2 / PROBLEMA 1, exclui execuções com erro) com 14 testes; adapter OpenAI via Responses API (tool `web_search` só no modo LIVE_SEARCH, extrai `url_citation`, estima custo) atrás de um registry plugável; `runRaioX` roda `perguntas × provedores × modos` com cooldown 1h/empresa e teto de 25 perguntas, persiste em `$transaction` e audita. **Stack do guia (Python/FastAPI/Pinecone) foi traduzida para Next 16 + Server Actions + Prisma** conforme CLAUDE.md; chatbot RAG e provedores Gemini/Perplexity/Claude ficaram fora do escopo (ver `.claude/context/geo/changelog.md`) |
| 2026-07-14 | `lib/security/safe-fetch.ts`, `seo/infra/fetchTargetPage.ts`, `seo/domain/seoCheck.ts`, `seo/actions/analyzeSeoUrl.ts`, `seo/queries/getSeoAnalysis.ts`, `dashboard/seo/[analysisId]/page.tsx`, `SeoUrlInputForm.tsx`, `SeoGeoPreview.tsx` | **FIX(seo/geo — anti-bot):** site com WAF/anti-bot (ex: fulltime.com.br) abortava a análise inteira; `safeFetch` ganhou opção `userAgent` e `fetchTargetPage` faz fallback com UA de navegador em 403/429 e, se ainda bloqueado, degrada (`contentAccessible:false`, html vazio) ainda pontuando sinais técnicos acessíveis (HTTPS/robots.txt/sitemap) em vez de erro; flag persistida em `checks` e banner âmbar na UI. Só `BLOCKED_BY_TARGET` degrada — TIMEOUT/UNREACHABLE/SSRF seguem erro |
| 2026-07-14 | `getUserCompanies.ts`, `signInAction.ts`, `dashboard/page.tsx`, `dashboard/seo/page.tsx`, `dashboard/seo/[analysisId]/page.tsx` | **FIX(multi-tenant + privacidade):** switcher/`/select-company` aparecia p/ usuário com 1 empresa vinculada quando o `companyId` primário divergia — `getUserCompanies` e `signInAction` agora contam só empresas **vinculadas** (`UserCompany`), primário como fallback sem vínculo (consistente com o que `/select-company` já fazia); "Atividade recente" e telas de SEO agora escopam pelo usuário **efetivo** (impersonado quando há impersonation), fechando o vazamento de atividade/relatórios de outros ao inspecionar um usuário |
| 2026-07-14 | `dashboard/layout.tsx`, `restartOnboarding.ts`, `updatePreferences.ts` | **FIX(onboarding — impersonation):** "Refazer tour" não funcionava ao inspecionar um usuário — as prefs eram gravadas/lidas no admin e o layout bloqueava o tour com `!impersonatedUserId`; agora `restartOnboarding`/`updatePreferences` gravam no usuário efetivo (impersonado) e o layout lê `preferences.onboarding` do usuário inspecionado (removido o gate de impersonação) |
| 2026-07-14 | `seo/domain/siteScan.ts`+`infra/fetchSitemapUrls.ts`+`actions/analyzeSite.ts`+`queries/getRecentSiteScans.ts`/`getSiteScan.ts` (novos), `getRecentSeoAnalyses.ts`, `components/seo/SiteScanButton/SiteScanReport/SiteSeoLanding/SiteScanReportView.tsx` (novos), `sites/[siteId]/seo/**`+`landing-pages/[lpId]/seo/**` (novas rotas), `Sidebar.tsx` | **FEAT(seo/geo — scan de site):** analisador SEO/GEO embutido em cada site e landing page (item "SEO/GEO" no PROJECT_ITEMS da Sidebar) que analisa **todas as páginas** de uma vez — descoberta = Pages publicadas (DB) ∪ sitemap.xml (dedup, mesma origem, teto 20), fetch concorrente (pool 5), score SEO+GEO por página → relatório agregado (média) + problemas recorrentes + drill-down por página. Persistência sem migração: reusa `SeoAnalysis` com `projectId` setado e breakdown em `checks.site`; `getRecentSeoAnalyses` (navbar) passou a filtrar `projectId: null` p/ não vazar scans de site; rate limit próprio de 10 scans/dia por site |
| 2026-07-14 | `dashboard/layout.tsx` | **FEAT(onboarding):** ADMIN/DEVELOPER também podem rodar o tour via "Refazer tour" (removido `!isPrivileged` da condição; segue bloqueado durante impersonação; admins nunca recebem `pending` automático — só via botão) |
| 2026-07-14 | `getRecentCompanyActivity.ts`, `dashboard/page.tsx`, `restartOnboarding.ts` (novo), `settings.client.tsx` | **FIX(privacidade+onboarding):** "Atividade recente" da home ganhou o mesmo escopo por usuário do módulo seo (param `userId` opcional no where do AuditLog; usuário comum só vê a própria atividade, ADMIN/DEVELOPER veem tudo); "Refazer tour" não funcionava — `router.push` navegava com router cache stale e o layout não via a preferência nova; substituído por Server Action `restartOnboarding` (Zod → auth → Prisma → revalidatePath → `redirect()` server-side), garantindo render fresco com o tour montado |
| 2026-07-14 | `getSeoAnalysis.ts`, `getRecentSeoAnalyses.ts`, `dashboard/seo/page.tsx`, `dashboard/seo/[analysisId]/page.tsx`, `dashboard/page.tsx`, `SeoAnalyzerCard.tsx` | **FIX(seo — privacidade):** usuários da mesma empresa viam análises uns dos outros; queries ganharam param opcional `userId` e as pages passam `session.user.id` quando `!isPrivilegedRole` (usuário comum só vê as próprias análises; URL direta de análise alheia → 404; ADMIN/DEVELOPER veem todas da empresa) |
| 2026-07-14 | `first-access/FirstAccessClient.tsx` | **UX(first-access):** tela responsiva (min-h-svh, paddings/tipografia mobile-first, logo escala w-24→w-28); lista estática de regras virou checklist ao vivo (marca cada requisito conforme digita, espelhando o `passwordSchema`); toggle mostrar/ocultar senha nos 2 campos (touch target 44px); indicador "As senhas coincidem"; `autoComplete="new-password"` |
| 2026-07-14 | `src/components/onboarding/*` (novos), `resetPasswordFirstAccess.ts`, `Sidebar.tsx`, `dashboard/layout.tsx`, `settings.client.tsx`, `next-auth.d.ts`, `globals.css` | **FEAT(onboarding):** tour de boas-vindas premium no 1º login pós-redefinição de senha (só usuários de empresa, sem impersonação): `resetPasswordFirstAccess` grava `preferences.onboarding={status:'pending',step:0}`; layout do dashboard monta `<OnboardingTour>` quando pending; Sidebar ganhou `data-tour` nos 7 itens (5 MAIN + Notificações + Configurações, só no desktop não-embedded); passo interativo usa `analyzeSeoUrl` REAL (sem mock); Configurações → Preferências ganhou "Refazer tour" (reseta pref + redireciona); animações 100% CSS (novos keyframes `onb*` em globals.css, com `prefers-reduced-motion`), zero dependência nova |
| 2026-07-14 | `dashboard/seo/page.tsx`, `SeoUrlInputForm.tsx` | **UX(seo/geo):** página índice usa largura total (removido `max-w-3xl`); cabeçalho com ícone em badge; "Análises recentes" virou grid responsivo (1/2/3 colunas) com entrada escalonada em vez de lista de 1 coluna; novo prop `layout?: 'compact'\|'full'` em `SeoUrlInputForm` — `'full'` (só na página índice) limita a barra de busca a `max-w-xl` e mostra os 2 checklists (SEO/GEO) lado a lado em `lg:grid-cols-2`; card da home (`SeoAnalyzerCard`) mantém `'compact'` (empilhado), sem mudança de comportamento |
| 2026-07-14 | `Sidebar.tsx`, `dashboard/seo/page.tsx`, `SeoAnalyzerCard.tsx` | **UX(seo/geo):** label "Análise de SEO e GEO" → "Análise SEO/GEO" (sidebar, h1 e card da home) |
| 2026-07-14 | `safe-fetch.ts`, `analyzeSeoUrl.ts`, `useCountUp.ts`/`ScoreRing.tsx`/`CombinedScoreHeader.tsx` (novos), `SeoScoreCard.tsx`, `SeoUrlInputForm.tsx`, `dashboard/seo/[analysisId]/page.tsx`, `Sidebar.tsx` | **FIX+UX(seo/geo):** fetch em produção falhava p/ alguns sites (ex: fulltime.com.br) com erro genérico "URL inacessível" — causa real era descartada sem log; agora `SafeFetchError` preserva `.cause`, loga o código de erro real, tenta 1 retry (falha de rede), e usa `dns.setDefaultResultOrder('ipv4first')` (via oficial do Node — **NÃO** usar `Agent`/`dispatcher` do pacote `undici` via npm, testado e incompatível com o fetch nativo, ver changelog do módulo); novo código `BLOCKED_BY_TARGET` p/ HTTP 403/429 do site-alvo, distinto de site fora do ar. Redesign: renomeado p/ "Análise de SEO e GEO" em toda a navegação; `CombinedScoreHeader` mostra os 2 scores animados (count-up) lado a lado no topo, clicáveis → scroll direto pro checklist; scroll automático ao concluir análise inline |
| 2026-07-13 | `src/modules/seo/domain/geoFoundationScoring.ts` (novo), `seoCheck.ts`, `parseHtml.ts`, `fetchTargetPage.ts`, `analyzeSeoUrl.ts`, `getSeoAnalysis.ts`, `SeoUrlInputForm.tsx`, `dashboard/seo/[analysisId]/page.tsx` | **FEAT(geo — task 15 fase 3 "Fundação Técnica"):** novo motor `scoreGeoFoundation()` (3 checks automatizados somando 100: robôs de IA liberados no robots.txt — GPTBot/ClaudeBot/PerplexityBot/Google-Extended etc., Organization/LocalBusiness, FAQPage — + 1 item editorial manual sem pontuação); reaproveita 100% do fetch SSRF-safe e parsing da task 11a (sem lib nova, sem migration nova); `checks` da `SeoAnalysis` passa a ser `{ seo, geoFoundation }` com leitura retrocompatível do array legado; relatório e preview inline ganham seção "Fundação para GEO" reaproveitando o `SeoScoreCard`; 13 testes novos |
| 2026-07-13 | `dashboard/seo/page.tsx` (novo), `Sidebar.tsx` | **FEAT(seo):** análise de SEO deixa de existir só na home — nova página índice `/[companySlug]/dashboard/seo` (form de URL + histórico completo de até 20 análises) e item "Análise de SEO" adicionado ao MAIN_ITEMS da Sidebar (ícone Gauge, ativo também em `/dashboard/seo/[analysisId]`) |
...
| 2026-07-06 | `NotificationsFeed.tsx` (substitui `ReleaseList.tsx`), `NotificationsContent.tsx`, `NotificationsSkeleton.tsx`, `api/notifications/releases/route.ts`, `getReleases.ts`, 3 sidebars/layouts/páginas | **FEAT(notifications):** timeline por mês, busca + filtros, "Carregar mais" paginado (GET route), Suspense+skeleton, badge numérico de não lidas no sino (`countUnreadReleases`); FIX: "Leia mais" cortava o fim (buffer 48px sobre scrollHeight p/ margens não contadas) |
| 2026-07-06 | `ReleaseBody.tsx`, `ReleaseList.tsx`, `globals.css` (keyframe `releaseCardIn`), 3 páginas de notificações | **UX(notifications):** página em largura total (removido max-w-4xl); cards com entrada escalonada (fade+slide) e hover; expansão do "Leia mais" animada (max-height + ResizeObserver, chevron 180°, fade-out do gradiente) |
| 2026-07-06 | `ReleaseBody.tsx`, `MarkNotificationsSeen.tsx`, `markNotificationsSeen.ts`, `hasUnreadReleases`, 3 sidebars/layouts/páginas, `next-auth.d.ts` | **FEAT(notifications):** descrição clampada em 3 linhas com "Leia mais"; lido/não-lido via `preferences.notifications_last_seen_at` com bolinha vermelha no sino; guia de extensão em `.claude/context/notifications/patterns.md` |
| 2026-07-06 | `refreshReleases.ts`, `RefreshReleasesButton.tsx`, 3 páginas de notificações | **FEAT(notifications):** botão "Atualizar" que expira o cache das releases via `updateTag('github-releases')` — release nova aparece na hora, sem esperar o TTL de 1h |
| 2026-07-06 | `ReleaseList.tsx`, `AdminSidebar.tsx`, `DevSidebar.tsx`, páginas/layouts de notificações admin e dev | **FEAT(notifications):** UI de releases extraída para `ReleaseList` (compartilhada); Notificações + versão no rodapé também nas sidebars Admin (`/dashboard-admin/notifications`) e Dev (`/dev/[devId]/dashboard/notifications`) |
| 2026-07-06 | `src/modules/notifications/*`, `dashboard/notifications/page.tsx`, `Sidebar.tsx`, `dashboard/layout.tsx` | **NEW(notifications):** módulo de notificações com releases do GitHub (`getSystemReleases`/`getCurrentVersion`, cache 1h); página `/[companySlug]/dashboard/notifications`; sino da Sidebar vira link e rodapé mostra versão atual |
