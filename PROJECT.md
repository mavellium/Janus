...
- **Actions:** `analyzeSeoUrl.ts` — Zod → auth → guard empresa → rate limit 20/dia → fetch SSRF-safe → parse → score SEO + score GEO foundation → persiste `SeoAnalysis.checks` como `{ seo, geoFoundation }` (análise avulsa: `projectId` fica null)
- **Queries:** `getSeoAnalysis.ts` — por id escopado a companyId, `parseStoredChecks()` lê tanto o formato novo `{seo,geoFoundation}` quanto o array legado pré-GEO | `getRecentSeoAnalyses.ts` — histórico avulso (`projectId: null`, não vaza scans de site) | `getRecentSiteScans.ts`/`getSiteScan.ts` — scans por `projectId`, lêem `checks.site`
