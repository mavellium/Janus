...
- **Actions:** `analyzeSeoUrl.ts` — Zod → auth → guard empresa → rate limit 20/dia → fetch SSRF-safe → parse → score SEO + score GEO foundation → persiste `SeoAnalysis.checks` como `{ seo, geoFoundation }`
- **Queries:** `getSeoAnalysis.ts` — por id escopado a companyId, `parseStoredChecks()` lê tanto o formato novo `{seo,geoFoundation}` quanto o array legado pré-GEO | `getRecentSeoAnalyses.ts` — histórico
...
- `src/components/seo/SeoUrlInputForm.tsx` — Client — input de URL (normaliza sem protocolo p/ https), useActionState → `analyzeSeoUrl`, resultado inline (SeoScoreCard SEO + SeoScoreCard Fundação GEO) + link p/ relatório
