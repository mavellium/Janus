...
- `src/components/landing/LandingNav.tsx` — Client — nav fixa da home pública; fundo **sólido** (`lp-nav-blur`) após 12px de scroll, âncoras de seção, drawer mobile (Esc fecha), logo light/dark por classe
...
- `src/components/landing/LandingFooter.tsx` — Server — rodapé da home pública com logo, colunas de âncoras e link de acesso

---

## Páginas

- `src/app/page.tsx` — Server — **landing pública do produto** (hero animado, CMS, SEO/GEO, Raio-X de IA, operação, fluxo, API, FAQ, CTA); com sessão ativa redireciona para `/dev/[id]/dashboard` (DEVELOPER), `/dashboard-admin` (ADMIN), `/no-company` (sem empresa) ou `/{companySlug}/dashboard`. O middleware já redireciona logado antes de renderizar. **Docs:** `.claude/context/landing/`
...
- **GeoTargetProfile** (`geo_target_profiles`) — id (UUID), name, description?, industry?, location?, website?, aliases (String[]), targetAudience?, differentiators?, createdById? (fk→users **SET NULL**), archivedAt? (soft delete), createdAt, updatedAt | índice [archivedAt]. **Não depende de `Company`** — perfil de qualquer negócio analisado, estilo conta HubSpot
